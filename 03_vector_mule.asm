...
main:      # ************* Code starts here ***********
           ...
           # $s0 ############################################################
           # Getting even bytes from $t0 
           sll $t5, $t0, 16		# left two bytes 	
           srl $t5, $t5, 24		# right three bytes
           srl $t4, $t0, 24	   	# right three bytes

           # Getting even bytes from $t2 
           sll $t7, $t2, 16		 	
           srl $t7, $t7, 24	
           srl $t6, $t2, 24	   	

           # Multiply even bytes and save them in lower 32 vector d ($s0)

           mult $t4, $t6		# $t4 = s0[0]*s0[2] 
           ...
           # $s1 ############################################################
           # Getting even bytes from $t1 
           sll $t5, $t1, 16		# left two bytes 	
           srl $t5, $t5, 24		# right three bytes
           srl $t4, $t1, 24	   	# right three bytes

           # Getting even bytes from $t3 
           sll $t7, $t3, 16		 	
           srl $t7, $t7, 24	
           srl $t6, $t3, 24	   	
